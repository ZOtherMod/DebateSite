...
class DebatePlatformServer:
    ...
    async def start_server(self):
        try:
            ...
            if os.getenv('PORT'):
                print("Production mode: WebSocket server only")
                ...
                print("Note: Static files should be served by Render's static hosting")
                
                await self.server.wait_closed()
                    
            else:
                print("Development mode: WebSocket server only")
                ...
                print("✓ Matchmaking service running")
                print("✓ Database initialized")
                print("Note: Serve frontend files separately (e.g., python -m http.server 8080)")
